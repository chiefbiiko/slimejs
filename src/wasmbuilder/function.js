...
export default class FunctionBuilder {
  ...
  getBody() {
    const locals = this.locals.map(l => [...varuint32(l.length), typeCodes[l.type]])

    const body = [...varuint32(this.locals.length), ...[].concat(...locals), ...this.code, 0x0b]
    return [...varuint32(body.length), ...body]
  }
