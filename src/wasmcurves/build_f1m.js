...
export default function buildF1m(module, _q, _prefix, _intPrefix) {
  ...
  function buildSign() {
    ...
    f.addCode(
      c.if(c.call(intPrefix + "_isZero", c.getLocal("x")), c.ret(c.i32_const(0))),
      c.call(prefix + "_fromMontgomery", c.getLocal("x"), AUX),
      c.if(c.call(intPrefix + "_gte", AUX, c.i32_const(pePlusOne)), c.ret(c.i32_const(-1))),
      c.ret(c.i32_const(1))
    )
  }
  ...
  function buildMReduct() {
    ...
    for (let i = 0; i < n32; i++) {
      ...
      f.addCode(
        c.setLocal(
          "m",
          c.i64_and(c.i64_mul(c.i64_load32_u(c.getLocal("t"), i * 4), c.getLocal("np32")), c.i64_const("0xFFFFFFFF"))
        )
      )
      ...
      for (let j = 0; j < n32; j++) {
        f.addCode(
          c.setLocal(
            "c",
            c.i64_add(
              c.i64_add(c.i64_load32_u(c.getLocal("t"), (i + j) * 4), c.i64_shr_u(c.getLocal("c"), c.i64_const(32))),
              c.i64_mul(c.i64_load32_u(c.i32_const(pq), j * 4), c.getLocal("m"))
            )
          )
        )
    ...
    f.addCode(
      c.call(prefix + "_add", c.i32_const(carries), c.i32_add(c.getLocal("t"), c.i32_const(n32 * 4)), c.getLocal("r"))
    )
  }
  ...
  function buildMul() {
    ...
    for (let k = 0; k < n32 * 2 - 1; k++) {
      ...
      if (k < n32) {
        f.addCode(
          c.setLocal(
            "m" + k,
            c.i64_and(
              c.i64_mul(c.i64_and(c.getLocal(c0), c.i64_const(0xffffffff)), c.getLocal("np32")),
      ...
      if (k >= n32) {
        f.addCode(c.i64_store32(c.getLocal("r"), (k - n32) * 4, c.getLocal(c0)))
      }
      ;[c0, c1] = [c1, c0]
  ...
  function buildSqrt() {
    ...
    f.addCode(
      // If (n==0) return 0
      c.if(c.call(prefix + "_isZero", c.getLocal("n")), c.ret(c.call(prefix + "_zero", c.getLocal("r")))),
  ...
  function buildIsSquare() {
    ...
    f.addCode(
      c.if(c.call(prefix + "_isZero", c.getLocal("n")), c.ret(c.i32_const(1))),
    ...
    f.addCode(
      ...
      c.block(
        c.loop(
          ...
          c.i32_store8(c.getLocal("j"), pAux, c.i32_load8_u(c.getLocal("p"))),
  ...
  buildExp(module, prefix + "_exp", n8, prefix + "_mul", prefix + "_square", intPrefix + "_copy", prefix + "_one")
